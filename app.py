...
GEOSERVER_LOCATION = os.getenv(
    "GEOSERVER_LOCATION", "http://localhost/geoserver14-geonode/"
)
...
@app.callback(
    ...
    [Input("bar-chart", "clickData"), Input("layer-selector", "value")],
)
def country_years_chart(selected_data, selected_layer):
    title = INDICATORS[selected_layer]["title"]
    if selected_data:
        country = selected_data["points"][0]["y"]
        df = get_filtered_data_frame(selected_layer, "iso3", country)
        ...
        mapbox=dict(
            layers=[
                dict(
                    ...
                    source=[
                        build_wms_layer_url(
                            WMS_URL, selected_layer, CQL_FILTER=wms_filter
                        )
                    ],
                    type="raster",
                    visible=True,
